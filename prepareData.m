...
%     if IDNames{i}(3)~='6'
    ...
    for j=15:L-5
        ...
        if low(j)<low(lowest(j+1,j+4))&& datenum(date(j+4))-datenum(date(j))<=10&&low(lowest(j-2,j))<=low(lowest(j-6,j))%&&high(j)<high(highest(j-2,j-1))
            upratio=( high(highest(j+1,j+4))-high(j) )/high(j);
            if upratio>0.25
                y(j-14)=1;
%                 figure;
