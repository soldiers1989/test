function [data,stock]=test_Spring(StockName) % modify stop loss point : when up over middle high point, modify stop loss as high point.
...
for i=200:L 
    ...
    if Sig(1)==0   % if there is not order which is hold.
        re1=Model_Spring(i); % tested function;
%         WD=weekday(date(i))-1;
        if re1>0 % && WD==4

            Sig(1)=1;
            Sig(2)=re1; % open price
            Sig(3)=re1; %low(lowest(i-3,i))*0.97;  % stop price
            Sig(4)=i;   % open bar;
            Sig(5)=i; % stop bar;
...
%     end
    ...
    orders=sum(Capi~=0);
    Orders=winN+lossN;
    if orders>20 % just consider the situation of generating at least 15 orders.
