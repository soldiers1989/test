function scanweek( varargin)   % put in date in varargin to decide which day as final day.
...
for i=1:length(IDNames)
    ...
    Vol=Data{6}(1:L);
    if ~isempty(Date)
        [Date,Open,High,Low,Close,Vol]=ReSampleUsed('week',datenum(Date),Open,High,Low,Close,Vol);
    else
        continue
    end
    L=length(Date);
