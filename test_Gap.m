function [data,stock]=test_Gap(StockName) % modify stop loss point : when up over middle high point, modify stop loss as high point.
...
for i=200:L 
    ...
    if Sig(1)==0   % if there is not order which is hold.
        re1=Model_Gap(i); % tested function;
%         WD=weekday(date(i))-1;
        if re1>0  % && WD==4
