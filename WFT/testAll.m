...
for loop=1:numStocks % loop for all stocks;
    ...
    for i=8:L-3
        ...
        if close(i-1)>open(i-1) && close(i-2)>open(i-2) && close(i-3)>open(i-3) && close(i-4)>open(i-4) &&...% continuous ups(close>open)
                ...
                close(i)<low(i)+0.25*(high(i)-low(i)) %(high(i)-low(i))>0.85*diff &&close(i) < low(i-4)+0.6*(high(i-1)-low(i-4))&&
            x(j)=i;
            y(j)=low(i);
            if close(i+1)<close(i) && close(i+2)<close(i+1) && low(i+2)>min(low(i-7:i)) 
                results(j)=0.5*(close(i+3)-close(i+2))/close(i+2);
                j=j+1;
            end
        end
    end   
...
% stocksGroup5=[stocks(indTem)',stocksAdd];
