...
should run matlab file:TestModelSpring (sw1) first; this is just middle procedure;
...
Re1=dataTem[0]  # close(ii+2)/close(ii)-1.003
Re1=dataTem.iloc[:,1] #close(ii+3)/close(ii)-1.003
Re2=dataTem.iloc[:,-2] #close(ii+2)/close(ii+1)-1.003

Date=dataTem.iloc[:,-1] # date of this day
factor1 = dataTem[2] # (close(ii)-low(ii))/(high(ii)-low(ii))
...
factor8 = dataTem[9] # close(ii)/low(ii-1)
factor9 = dataTem[10] # close(ii)/high(ii-1)
factor10 = dataTem[11] # close(ii)/mean([low(ii-1),high(ii-1)]),
X = np.column_stack([factor2,factor3,factor5,factor8,factor9]) #
X,X0,Re1,y0=train_test_split(X,Re1,test_size=0.0)

#hmm=joblib.load('D:\Trading\Python\machinelearning\HMMTest') # for matlab TestModelSPring (sw2)
#latent_states_sequence=hmm.predict(X)  
#indTem=(latent_states_sequence==1)
#ReSelected=Re1[indTem]
#Re2Selected=Re2[indTem]
#DateSelected=Date[indTem]
#fout=open('D:\Trading\hmmMatlabOut.txt','w')
#for i in range(len(ReSelected)):
#    fout.writelines('%.5f,%.5f,%d\n' %(ReSelected.iloc[i],Re2Selected.iloc[i],DateSelected.iloc[i])) #indTem.astype(int)
#fout.close()


#sort
#hmm=GaussianHMM(n_components=5,covariance_type='diag',n_iter=20000).fit(X) #spherical,diag,full,tied 
#joblib.dump(hmm,'HMMTest')
#hmm=joblib.load('D:\Trading\Python\machinelearning\HMMTest')
#joblib.dump(hmm,'HMMTem')
hmm=joblib.load('D:\Trading\Python\machinelearning\HMMTem')
...
buy=(latent_states_sequence==3)+(latent_states_sequence==1)+(latent_states_sequence==0)+(latent_states_sequence==15)
...
#X = np.column_stack([factor1,factor3,factor5,factor6])
#X=X[indTem]
#Re1=Re1[indTem]
#
