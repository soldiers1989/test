        ...
        if high(L-j)>=high(highest(L-ceil(3*j/2),L))
            for jj=L-3*j-3:L-j-1
                ...
                try
                    ...
                    if high(highest(jj-j-1,jj-j+1))>=high(highest(startTem,L))...% first point;
                            && low(lowest(jj-j,L)) >= low(lowest(jj-4*j,jj-j))... % first point;
                            && low(jj)<=low(lowest(jj-ceil(j/2),L-j))...% second point;
                            && high(L-j)>=max(high(jj:L))... % 3rd point; 
                            && low(L)<=low(lowest(jj-j,L))... % 4th point;
                            && L-j-jj>3 % up-bars more than 3;
                        ...
                        if (high(H1)-low(L1))*1.3/(L1-H1)>=(high(H2)-low(L))/(L-H2) && ( abs(L-2*j-jj)<=1 || low(lowest(L-j,L))>=low(lowest(jj-j,jj)) )%&& AboveHalf(Bar0) 
                            kline=kline+1;
